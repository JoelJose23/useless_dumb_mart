...
import MemeExplosion from './MemeExplosion' // Make sure this path matches your project structure!
...
function AcademicCaptcha({ onSuccess, onFailure }) {
  ...
  // NEW: State to trigger the meme explosion.
  // Using a number allows us to force a re-render if they spam wrong answers.
    ...
    const timer = setInterval(() => {
      setTimeLeft((time) => {
        if (time <= 1) {
          ...
          // Notify parent, but DO NOT immediately unmount
          // the CAPTCHA. We want the glorious failure screen.
          onFailure?.()
  ...
  const handleSubmit = () => {
    const userAnswer = normalizeAnswer(answers[currentQuestion])
    ...
    if (userAnswer !== correctAnswer) {
      ...
      // TRIGGER THE MEME EXPLOSION!
      setExplosionKey((prev) => prev + 1)

      if (attempt >= 3) {
        setStatus('failed')
        // Keep failure screen mounted.
        onFailure?.()
      } else {
  ...
  if (status === 'success') {
    return (
      <div className="captcha-box captcha-success">
        ...
        <div className="captcha-label">EVOLUTIONARY ASSESSMENT COMPLETE</div>
        <h2>YOU DID IT! 🎉</h2>
        <p>Congratulations, Homo sapiens!</p>
        <p>You have successfully demonstrated enough cognitive activity to purchase completely unnecessary merchandise.</p>
        <p>We are incredibly proud of you.</p>
        <p>Seriously. We weren't sure this would happen.</p>
        <p className="captcha-small-text">Your ancestors would be moderately proud. Probably.</p>
        <button onClick={onSuccess}>BE PROUD AND BUY YOUR PRICE</button>
      </div>
    )
  }
  ...
  if (status === 'failed') {
    return (
      <>
        {/* Render the explosion even on the failure screen if it just triggered */}
        {explosionKey > 0 && (
          <MemeExplosion key={explosionKey} onDone={() => setExplosionKey(0)} />
        )}
        <div className="captcha-box captcha-failed">
          ...
          <h2>GREAT NEWS! 🌟</h2>
          <p>YOU TRIED! 🎊</p>
          <p>And honestly, that is what really matters.</p>
          <p>Unfortunately, your answers suggest that your evolutionary journey may require additional time.</p>
          <p>Please don't be discouraged. The Quantum Rock™ believes in you.</p>
          <p>Unfortunately, the Quantum Rock™ is also inanimate, so its opinion may not be legally meaningful.</p>
          <p className="captcha-small-text">Classification: temporarily ape. Potential for greatness: under investigation.</p>
  ...
  return (
    <>
      {/*
        This is where the magic happens. We pass explosionKey as the React key.
        When it increments, React destroys the old explosion and mounts a fresh one.
      */}
      {explosionKey > 0 && (
        <MemeExplosion key={explosionKey} onDone={() => setExplosionKey(0)} />
      )}
      ...
      <div className="captcha-box">
        <div className="captcha-header">
          <div>
            <span className="captcha-label">✨ MANDATORY EVOLUTIONARY SCREENING ✨</span>
            <h2>Are You Worthy?™</h2>
            <p className="captcha-subtitle">We're so excited to see how far you've evolved!</p>
          </div>
          <div className="captcha-timer">{timeLeft}s</div>
        </div>

        <div className="captcha-meta">
          <span>🌱 evolutionary checkpoint {currentQuestion + 1}</span>
          <span>💖 attempts remaining: {4 - attempt}</span>
        </div>

        <div className="captcha-question">
          <div className="captcha-subject">{question.subject}</div>
        ...
        <button className="captcha-submit" onClick={handleSubmit}>
        ...
        <p className="captcha-warning">
          💚 Remember: every wrong answer is an opportunity to discover something new about yourself.
        </p>
